...
cdef class PureMarketMakingStrategy(StrategyBase):
    ...
    def did_start(self):
        ...
        if qty == 0:
            ...
            self.logger().info(f"Averaging with {qty} into {base_balance} with current price {current_price:.8g} to : {self._wac:.8g}")  # BYAO DEBU
        self.logger().info(f"_wac set to {self._wac:.8g}")  # BYAO DEBU
    ...
    def order_refresh_time(self, value: float):
        ...
        self._create_timestamp = 0  # force recreate
        self._cancel_timestamp = 0
    ...
    def pure_mm_assets_df(self, to_show_current_pct: bool) -> pd.DataFrame:
        ...
        data=[
            ...
            [f"Cost (WAC: {wac:.6g})", round(cost, 4), ""],
    ...
    cdef bint active_order_too_old(self):
        ...
        return int(time.time() - minCreationTime/1e6) > 600 #(1800 - 60) # 10 mins now
        ...
        if self._hanging_orders_enabled:
            ...
            if order_id in self._hanging_order_ids:
                ...
                self.notify_hb_app(
                    ...
                    f"{limit_order_record.price} {limit_order_record.quote_currency} is filled at {datetime.datetime.now().strftime('%H:%M:%S')}."
                )
                return
