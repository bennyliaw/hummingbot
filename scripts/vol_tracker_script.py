...
class VolTracker(ScriptBase):
    ...
    def on_tick(self):
        ...
        if self.prev_vol is None or avg_short_volatility is None or abs(avg_short_volatility - self.prev_vol) < 0.0002 :
            self.log(f"none or too small avg_short_volatility: {avg_short_volatility} prev_vol={self.prev_vol}")
            return
